...
class DES_A{
	...
	String encrypt(String plain , String key){
		...
		plain = permutation(IP , plain);
		for(int i = 0;i < 16;i++){
			plain = round(plain , keys[i] , i);
		}
		plain = plain.substring(8 , 16) + plain.substring(0 , 8);
		plain = permutation(IP1 , plain);
		return plain;
	}
	String decrypt(String cipher , String key){
		String keys[] = getKeys(key);
		cipher = permutation(IP , cipher);
		for(int i = 15;i > -1;i--){
			cipher = round(cipher , keys[i] , 15-i);
		}
		cipher = cipher.substring(8 , 16) + cipher.substring(0 , 8);
		cipher = permutation(IP1 , cipher);
		return cipher;
	}
}
...
public class DES{
	public static void main(String[] args){
		...
		DES_A d = new DES_A();
		String cipher = d.encrypt(plaintext , key);

		String original = d.decrypt(cipher , key);
